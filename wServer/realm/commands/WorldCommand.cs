...
namespace wServer.realm.commands
{
    ...
    // public void Execute(Player player, string[] args) { Dictionary<string, ICommand> cmds = new
    // Dictionary<string, ICommand>(); var t = typeof(ICommand); foreach (var i in
    // t.Assembly.GetTypes()) if (t.IsAssignableFrom(i) && i != t) { var instance =
    // (ICommand)Activator.CreateInstance(i); cmds.Add(instance.Command, instance); } StringBuilder
    // sb = new StringBuilder("Commands: "); var copy = cmds.Values.ToArray(); for (int i = 0; i <
    // copy.Length; i++) { if (i != 0) sb.Append(", "); sb.Append(copy[i].Command); }
    ...
    // public void Execute(Player player, string[] args) { List<BattleArenaMap> Arenas = new
    // List<BattleArenaMap>(); List<string> ArenaTexts = new List<string>();

    // ArenaTexts.Add("Current arenas:");

    // foreach (var w in RealmManager.Worlds) { World world = w.Value; if (w.Value.Name == "Battle
    // Arena" && w.Value.Players.Count > 0) { Arenas.Add(w.Value as BattleArenaMap); } } if
    // (Arenas.Count > 0) { foreach (var w in Arenas) { string ctext = "Wave "+ w.Wave +" - {0}
    // {1}"; List<string> players = new List<string>(); int solo = 0; foreach (var p in w.Players) {
    // players.Add(p.Value.Name); if (PlayerDataList.GetData(p.Value.Client.Account.Name).Solo)
    // solo++; } if (players.Count > 0) { ArenaTexts.Add(string.Format(ctext, string.Join(", ",
    // players.ToArray()), solo == players.Count ? " (SOLO)" : "")); } } }

    // if(ArenaTexts.Count == 1) ArenaTexts.Add("None");
