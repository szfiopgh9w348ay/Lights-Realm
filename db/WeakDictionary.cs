...
// used in place of a constructor to handle the case where target is null, but we want the reference
// to still appear to be alive.
    ...
    where T : class
{
    ...
    // 
    // Let Wa = Alive Weak Reference Let Wd = Dead Weak Reference Let S = Strong Reference
    // 
    // x | y | Equals(x,y) ------------------------------------------------- Wa | Wa |
    // comparer.Equals(x.Target, y.Target) Wa | Wd | false Wa | S | comparer.Equals(x.Target, y) Wd
    // | Wa | false Wd | Wd | x == y Wd | S | false S | Wa | comparer.Equals(x, y.Target) S | Wd |
    // false S | S | comparer.Equals(x, y) -------------------------------------------------
...
/// <summary> Represents a dictionary mapping keys to values. </summary>
/// 
/// <remarks> Provides the plumbing for the portions of IDictionary<TKey, TValue> which can
/// reasonably be implemented without any dependency on the underlying representation of the
/// dictionary. </remarks>
